...
export default function ServicesCategoriesPage() {
  ...
  const fetchAll = async (pageNum = 1) => {
    try {
      const res = await CategoryAdmin.listServices({ page: pageNum });
      setPage(pageNum);
  ...
  const handleSave = async (data: CategoryFormData) => {
    ...
    try {
      if (editing) {
        await CategoryAdmin.updateService(editing.id, payload);
        toast.success("Category updated");
      } else {
        await CategoryAdmin.createService(payload);
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await CategoryAdmin.deleteService(deleteId);
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 leading-none">Services Categories</h1>
          <p className="text-xs text-gray-500 mt-1">Categories / Services</p>
        </div>
