...
export default function BlogCategoriesPage() {
  ...
  const fetchAll = async (pageNum = 1) => {
    try {
      const res = await CategoryAdmin.listBlog({ page: pageNum });
      setPage(pageNum);
  ...
  const handleSave = async (data: CategoryFormData) => {
    ...
    try {
      if (editing) {
        await CategoryAdmin.updateBlog(editing.id, payload);
        toast.success("Category updated");
      } else {
        await CategoryAdmin.createBlog(payload);
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await CategoryAdmin.deleteBlog(deleteId);
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 leading-none">Blog Categories</h1>
          <p className="text-xs text-gray-500 mt-1">Categories / Blog</p>
        </div>
