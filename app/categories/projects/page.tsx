...
export default function ProjectCategoriesPage() {
  ...
  const fetchAll = async (pageNum = 1) => {
    try {
      const res = await CategoryAdmin.listProject({ page: pageNum });
      setPage(pageNum);
  ...
  const handleSave = async (data: CategoryFormData) => {
    ...
    try {
      if (editing) {
        await CategoryAdmin.updateProject(editing.id, payload);
        toast.success("Category updated");
      } else {
        await CategoryAdmin.createProject(payload);
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await CategoryAdmin.deleteProject(deleteId);
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 leading-none">Project Categories</h1>
          <p className="text-xs text-gray-500 mt-1">Categories / Project</p>
        </div>
