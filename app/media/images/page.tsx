...
export default function ImagesPage() {
  ...
  const { data, isLoading } = useMediaList({ page: currentPage, page_size: PAGE_SIZE, group_title: "Images" });
  ...
  const handleSave = async (formData: MediaFormData, files?: File[]) => {
    ...
    try {
      ...
      if (editing) {
        ...
        for (const f of list) {
          const result = await uploadMutation.mutateAsync({ file: f, metadata: { ...payload, group_title: "Images" } });
  ...
  return (
    <PageHeader title="Images" subtitle="Media / Images" actionLabel="Add Image" onAction={() => { setEditing(null); setView("form"); }}>
      <MediaTable
        items={items}
        page={currentPage}
