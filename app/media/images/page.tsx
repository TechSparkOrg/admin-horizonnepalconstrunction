...
export default function ImagesPage() {
  ...
  const fetchAll = async (pageNum = 1) => {
    try {
      const res = await MediaService.listImages({ page: pageNum });
      setPage(pageNum);
  ...
  const handleSave = async (data: MediaFormData, files?: File[]) => {
    ...
    try {
      ...
      if (editing) {
        ...
        for (const f of list) {
          await MediaService.uploadImage(f, { ...payload, group_title: 'Images' });
        }
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 leading-none">Images</h1>
          <p className="text-xs text-gray-500 mt-1">Media / Images</p>
        </div>
