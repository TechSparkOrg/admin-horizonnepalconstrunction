...
export function AgreementTable({ items, onEdit, onDelete, deleteId, setDeleteId, page, totalPages, onPageChange }: Props) {
  if (items.length === 0) {
    return (
      <Card className="bg-white border border-gray-200 rounded-xl">
        <CardContent className="p-16 text-center">
          ...
          <p className="text-sm font-medium text-gray-900 mb-1">No agreements yet</p>
          <p className="text-sm text-gray-500">Create an agreement to get started.</p>
        </CardContent>
      </Card>
    );
  }
          ...
          <TableBody>
            {items.map((item) => (
              ...
              >
                ...
                <TableCell>
                  <span className="text-sm text-gray-700">{item.client_name}</span>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <span className="text-sm text-gray-500">{item.project_name || "—"}</span>
                </TableCell>
                <TableCell>
                  <Badge
                    variant="outline"
                  ...
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${item.status === "completed" ? "bg-green-500" : "bg-amber-500"}`} />
